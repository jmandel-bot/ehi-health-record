...
Analyze the mapping pipeline for **History: SOCIAL_HX + SURGICAL_HX + FAMILY_HX** and identify every semantic error — places where a column is read successfully but the value means something different than the code assumes.
...
- **LINE**: The line number of the surgical history contact within the encounter.
- **PROC_ID**: The unique ID of the procedure record associated with the surgical history contact. Note: This is NOT the CPT(R) code. It is an internal identifier that is typically not visible to a user.
- **SURGICAL_HX_DATE**: The free-text date entered in a patient's Surgical History for the procedure.
- **COMMENTS**: Free-text comments entered for the procedure in the surgical history contact.
- **PROC_COMMENTS**: Free-text comments entered for the "other" procedure in the patient's surgical history.
- **CONTACT_DATE**: The contact date of the encounter associated with this surgical history contact. Note: There may be multiple encounters on the same calendar date.
- **CM_CT_OWNER_ID**: ID of the deployment owner for this contact.
- **PAT_ENC_CSN_ID**: A unique serial number for this encounter. This number is unique across all patients and encounters in the system.
- **SURGICAL_HX_SRC_C_NAME**: This category number represents the source of entry, the person providing the information, for the surgical history data.
- **HX_LNK_ENC_CSN**: The Contact Serial Number of the encounter in which the history was created/edited. If the history was created/edited outside of the context of an encounter, then this column will be blank.
- **SURGICAL_HX_SRG_CSN**: Stores the contact serial number of the surgery contact related to the current procedure.
- **SURG_LATERALITY_C_NAME**: This category is used to store the laterality for each past surgical record in surgical history.
- **SURG_HX_START_DT**: The starting date for the range of possible dates extracted from the free text date entered in column SURGICAL_HX_DATE. It is filled in automatically when the patient's history is saved. The ending date is in column SURG_HX_END_DT.
- **SURG_HX_END_DT**: The ending date for the range of possible dates extracted from the free text date entered in column SURGICAL_HX_DATE. It is filled in automatically when the patient's history is saved. The starting date is in SURG_HX_END_DT.
- **SURG_HX_ORDER_ID**: The unique ID of the order related to the current surgical history procedure
